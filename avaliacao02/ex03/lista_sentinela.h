...
// IMPLEMENTAR
    ...
    No* primeirob = b->sentinela->prox;
   
    while (lista_tamanho(b) > 0){     
        if(primeirob->dado > aux_a->dado && aux_a != a->sentinela){
            aux_a= aux_a->prox; 
        }else{
            desconecta_no(primeirob);
            if(primeirob->dado < aux_a->dado){
                insere_esquerda(aux_a,primeirob);
            }else{
                lista_inserirFim(a,primeirob->dado);
            }
            primeirob = b->sentinela->prox;
            b->qtde--;
        }
    }
}
